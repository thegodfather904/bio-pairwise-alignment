...
import {
  PlotValue
} from './plot-value.model';
import {
  VisualizerData
} from './visualizer-data.model';

export class DovetailAlignment {
  constructor() {}
  ...
  fillOutPlot(vd: VisualizerData) {
    ...
    for (let row = 1; row < vd.sequence2.length + 1; row++) {
      ...
      for (let col = 1; col < vd.sequence1.length + 1; col++) {
        ...
        // if last columns, see if max value
        if (col === vd.sequence1.length) {
          if (maxPlotValue === null || maxPlotValue.score < maxScore) {
            maxPlotValue = currentPlotValue;
          }
        }
      }
      // if last row, see if max value
      if (row === vd.sequence2.length) {
        for (let col = 0; col < vd.sequence1.length + 1; col++) {
          if (maxPlotValue.score < this.plotMatrix[row][col].score) {
            maxPlotValue = this.plotMatrix[row][col];
          }
        }
      }
    }
